...
This document outlines the steps to release a new version of **azure-functions-db** to PyPI and update the changelog using the existing Makefile and Hatch-based workflows.
...
1. Update the version in `src/azure_functions_db/__init__.py`
2. Generate or update `CHANGELOG.md` via `git-cliff`
3. Commit the version bump and changelog
4. Create a Git tag (e.g., `v0.2.0`) and push to `main`
