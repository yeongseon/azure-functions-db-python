...
`azure-functions-db-python` 提供了面向后续绑定的共享基础设施。使用 `DbConfig` 统一连接配置；当多个组件需要复用惰性创建的 SQLAlchemy 引擎时，使用 `EngineProvider`。
...
`azure-functions-db-python` 提供结构化日志辅助能力以及轻量级 `MetricsCollector` 协议，可接入你自己的指标后端而无需引入重依赖。
